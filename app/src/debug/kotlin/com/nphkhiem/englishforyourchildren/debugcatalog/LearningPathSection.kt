...
/**
 * One unit story page at a time.
 *
 * Worth walking with two questions. Does Pip stand over the lesson a child should open next, and
 * do the lessons still ahead read as later rather than as refusals? Focus never reaches those, so
 * the only thing carrying their meaning is how they look.
 *
 * The stepper reports its action rather than paging here, in the same way child home reports
 * Continue. Paging itself is asserted by the instrumented tests.
 */
@Composable
internal fun LearningPathSection() {
    val states = remember { LearningPathFixtures.reviewStates() }
    var index by remember { mutableIntStateOf(0) }
    var lastAction by remember { mutableStateOf<LearningPathAction?>(null) }
    val (stateName, state) = states[index]

    Column(verticalArrangement = Arrangement.spacedBy(HelloBeSpacing.space4)) {
        Text(
            text = stringResource(R.string.theme_catalog_path_label),
        ...
        Box(
            ...
            LearningPathScreen(state = state, onAction = { lastAction = it })
        }
    }
}
